...
shapez.SerializerInternal.prototype["deserializeEntityNoPlace"] = function (
  root,
  payload
) {
  ...
  let errorStatus = this.deserializeComponents(
    root,
    entity,
    payload.components
  );
...
function deserializeBlueprintString(root, blueprint) {
  try {
    ...
    let data = blueprint.substring(
      BP_PREFIX.length + 1,
      blueprint.length - BP_SUFFIX.length
    );
...
function writeValue(value, type) {
  if (type === "boolean_item") {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      data[Math.floor(pos / 8)] |=
        offset < 0 ? pair << -offset : pair >> offset;
      if (offset > 0) {
    ...
    this.modInterface.runAfterMethod(
      ...
      function () {
        document.addEventListener("paste", (ev) => {
          if (
            this.root.app.inputMgr.getTopReciever().context ===
            "state-InGameState"
          ) {
            let blueprint;
            try {

...
const METADATA = {
    ...
    version: "0.1",
    id: "big-displays",
    description: "Adds 1x16 display",
    minimumGameVersion: ">=1.5.0",
};

const DISPLAYS = { big16: { x: 16, y: 1 } };
...
class MetaBigDisplays extends shapez.ModMetaBuilding {
    ...
    getDimensions() {
        return new shapez.Vector(DISPLAYS.big16.x, DISPLAYS.big16.y);
    }
    ...
    setupEntityComponents(entity) {
        const slots = [];
        for (let x = 0; x < DISPLAYS.big16.x; x++) {
            for (let y = 0; y < DISPLAYS.big16.y; y++) {
                slots.push({
